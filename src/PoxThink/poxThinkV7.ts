...
export const PoxThinkV7 = (
  ...
  // --- Fonctions d'attaque et de génération de coups (V10) ---
  ...
  const getMovesForPiece = (p: Piece, x: number, y: number, color: GameColor): Move[] => {
    ...
    if (name === PieceName.pion) {
      ...
      for (const dx of [-1, 1]) {
        ...
        if (target && target.color !== color) { if (y + dir === promotionRank) { [PieceName.queen, PieceName.tour, PieceName.fou, PieceName.cavalier].forEach(p => addMove(x + dx, y + dir, p)); } else { addMove(x + dx, y + dir); } }
      }
      if (board.enPassantTarget && board.enPassantTarget.y === y + dir && (board.enPassantTarget.x === x - 1 || board.enPassantTarget.x === x + 1)) { addMove(board.enPassantTarget.x, board.enPassantTarget.y, undefined, undefined, true); }
  ...
  const quiescenceSearch = (alpha: number, beta: number, ply: number): number => {
    ...
    const movesToSearch = legalMoves.filter(move => {
      if (inCheck) return true; // Cherche toutes les évasions
      return move.capturedPiece || move.isEnPassant || move.promotion; // Sinon, que les captures/promotions
    });
  ...
  const negamax = (depth: number, alpha: number, beta: number, ply: number): number => {
    ...
    // ✨✅ CORRECTION V12: La variable manquante est ici
    const originalAlpha = alpha;

    if (ply > rootMaxDepth + MAX_EXTENSION_PLY) {
      return evaluate(); // Arrêt d'urgence de la boucle infinie
    }
    ...
    if (!inCheck && ttEntry && ttEntry.depth >= depth) { // Ne pas utiliser la TT si en échec (peut être instable)
    ...
    for (const move of moves) {
      ...
      if (score > alpha && moveCount > 3) { // Re-recherche
        score = -negamax(effectiveDepth - 1, -beta, -alpha, ply + 1);
      }
      board.unmakeMove();
    ...
    if (!timeUp) {
      ...
      if (alpha > originalAlpha) { flag = 'EXACT'; } // C'est ici que ça plantait
  ...
  // --- Fonction principale `thinkFor` (V11) ---
  const thinkFor = (color: GameColor): ThinkResponse['white'] => {
    ...
    for (let currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
      rootMaxDepth = currentDepth; // Définit la profondeur max pour les extensions

      // Vider les killers pour la nouvelle itération
      ...
      orderMoves(moves, ttMove, 0); // ply 0 pour la racine
      ...
      if (timeUp) {
        console.log(`PoxThinkV12: Time limit reached at depth ${currentDepth}. Nodes: ${nodes}`);
        break;
      }
      if (legalMovesFound === 0) {
        console.log(`PoxThinkV12: No legal moves found (Mate/Stalemate).`);
        bestMove = null;
        break;
      }
      if (currentBestMoveForDepth) {
        ...
        bestScore = alpha;
        console.log(`PoxThinkV12: Depth ${currentDepth} complete. Best move: ${bestMove.fromX},${bestMove.fromY} -> ${bestMove.toX},${bestMove.toY}. Score: ${bestScore}. Nodes: ${nodes}`);
      }
      if (bestScore > MATE_THRESHOLD) {
        console.log(`PoxThinkV12: Mate found at depth ${currentDepth}.`);
        break;
      }
    }
    ...
    // CORRECTION: Il y avait une faute de frappe ici dans V11 (`locY`)
    ...
    if (index === -1) {
      ...
      if (altIndex === -1) {
        console.error("PoxThinkV12: CRITICAL - Could not find index for best move.");
        return undefined; // Erreur critique
      }

...
export const PoxThinkV8 = (
  ...
  // --- ✨ V13 Génération de Coups (Corrigée) ---
  const getMovesForPiece = (p: Piece, x: number, y: number, color: GameColor): Move[] => {
    const generatedMoves: Move[] = [];
    const { name } = p;

    // Fonction d'aide pour les captures / coups spéciaux (Roi, Cavalier, etc.)
    const addMove = (toX: number, toY: number, promotion?: PieceName, isCastle?: 'K' | 'Q', isEnPassant?: boolean) => {
      ...
      // ✅ CORRECTION: S'assure qu'on ne bouge pas sur une pièce alliée
      if (target && target.color === color) {
        return; // Ne peut pas bouger sur une pièce alliée
      }

      const captured = target; // `target` est soit null, soit une pièce ennemie
      let score = 0;
    ...
    // Fonction d'aide pour les pièces qui glissent (Tour, Fou, Dame)
        ...
        while (curX >= 0 && curX <= 7 && curY >= 0 && curY <= 7) {
          ...
          if (target) {
            if (target.color !== color) {
              addMove(curX, curY); // Capture
            }
            break; // Bloqué par une pièce (alliée ou ennemie)
          }
          addMove(curX, curY); // Case vide
          curX += dx; curY += dy;
        }
      }
    };
    ...
    if (name === PieceName.pion) {
      ...
      // ✅ CORRECTION: Logique d'avancée directe (n'utilise plus addMove)
      // Avancer de 1
      if (toY >= 0 && toY <= 7 && !getPieceAt(x, toY)) {
        if (toY === promotionRank) {
          // Promotions
          [PieceName.queen, PieceName.tour, PieceName.fou, PieceName.cavalier].forEach(promoName => {
            const score = 900000 + PIECE_VALUE[promoName];
            generatedMoves.push({ fromX: x, fromY: y, toX: x, toY: toY, piece: name, promotion: promoName, score });
          });
        } else {
          // Avancée simple
          const score = historyTable[pieceIndexMap[name]][toY * 8 + x];
          generatedMoves.push({ fromX: x, fromY: y, toX: x, toY: toY, piece: name, score });
        }

        // Avancer de 2 (seulement si l'avancée de 1 est possible)
        if (y === startRank) {
          const toY2 = y + 2 * dir;
          if (!getPieceAt(x, toY2)) {
            const score = historyTable[pieceIndexMap[name]][toY2 * 8 + x];
            generatedMoves.push({ fromX: x, fromY: y, toX: x, toY: toY2, piece: name, score });
          }
        }
      }

      // Captures (utilise addMove car il y a une cible)
      for (const dx of [-1, 1]) {
        const toX = x + dx;
        if (toX < 0 || toX > 7) continue;

        const target = getPieceAt(toX, toY);
        if (target && target.color !== color) {
          if (toY === promotionRank) {
            [PieceName.queen, PieceName.tour, PieceName.fou, PieceName.cavalier].forEach(promoName => {
              const score = (1000000 + (PIECE_VALUE[target.name] - PIECE_VALUE[name])) + (900000 + PIECE_VALUE[promoName]);
              generatedMoves.push({ fromX: x, fromY: y, toX: toX, toY: toY, piece: name, capturedPiece: target.name, promotion: promoName, score });
            });
          } else {
            addMove(toX, toY); // Appel normal pour capture
          }
        }
      }

      // Capture "En Passant"
      if (board.enPassantTarget && board.enPassantTarget.y === toY) {
        if (board.enPassantTarget.x === x - 1 || board.enPassantTarget.x === x + 1) {
      ...
      // Logique pour Cavalier, Tour, Fou, Dame (V10)
  ...
  const quiescenceSearch = (alpha: number, beta: number, ply: number): number => {
    ...
    const allMoves = generateAllMoves(board.turn);
    ...
    const movesToSearch = legalMoves.filter(move => {
      if (inCheck) return true;
      return move.capturedPiece || move.isEnPassant || move.promotion;
    });

    orderMoves(movesToSearch, undefined, ply);

    for (const move of movesToSearch) {
      if (!inCheck && move.capturedPiece) {
  ...
  const negamax = (depth: number, alpha: number, beta: number, ply: number): number => {
    ...
    for (const move of moves) {
      ...
      if (moveCount > 3 && effectiveDepth >= 3 && !inCheck && !move.capturedPiece && !move.promotion && !move.isCastle) {
  ...
  // --- Fonction principale `thinkFor` (V12) ---
  const thinkFor = (color: GameColor): ThinkResponse['white'] => {
    ...
    for (let currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
      ...
      if (timeUp) {
        console.log(`PoxThinkV13: Time limit reached at depth ${currentDepth}. Nodes: ${nodes}`);
        break;
      }
      if (legalMovesFound === 0) {
        console.log(`PoxThinkV13: No legal moves found (Mate/Stalemate).`);
        bestMove = null;
        break;
      }
      if (currentBestMoveForDepth) {
        ...
        bestScore = alpha;
        console.log(`PoxThinkV13: Depth ${currentDepth} complete. Best move: ${bestMove.fromX},${bestMove.fromY} -> ${bestMove.toX},${bestMove.toY}. Score: ${bestScore}. Nodes: ${nodes}`);
      }
      if (bestScore > MATE_THRESHOLD) {
        console.log(`PoxThinkV13: Mate found at depth ${currentDepth}.`);
        break;
      }
    }
    ...
    if (index === -1) {
      ...
      if (altIndex === -1) {
        console.error("PoxThinkV13: CRITICAL - Could not find index for best move.");
        return undefined;
      }
  ...
  // ✅ CORRECTION: Faute de frappe `thinkA` corrigée
